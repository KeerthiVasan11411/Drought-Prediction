...
# Load dataset once for historical date lookups
...
def index():
    ...
    if request.method == 'POST':
        try:
            ...
            if matched.empty:
                # Fallback to general monthly average if exact day match isn't found
                matched = df_dataset[df_dataset['Month'] == month]

            # Calculate average weather/soil conditions for this period in Kancheepuram
            avg_rainfall = matched['Rainfall (mm)'].mean()
            avg_temp = matched['Temperature (°C)'].mean()
            avg_humidity = matched['Relative Humidity (%)'].mean()
            avg_soil = matched['Soil Moisture (%)'].mean()
            avg_wind = matched['Wind Speed (m/s)'].mean()
            avg_et0 = matched['Evapotranspiration ET0 (mm)'].mean()
            avg_drought_idx = matched['Drought Index (SPI-like)'].mean()
