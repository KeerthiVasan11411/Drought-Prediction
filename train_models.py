...
# 4. Train Models
print("Training Drought Classification Model...")
clf_model = RandomForestClassifier(n_estimators=100, random_state=42)
clf_model.fit(X, y_class_encoded)

print("Training Irrigation Recommendation Regression Model...")
reg_model = RandomForestRegressor(n_estimators=100, random_state=42)
reg_model.fit(X, y_reg)

# 5. Save Artifacts (Generates the .pkl files)
joblib.dump(clf_model, 'drought_classifier.pkl')
joblib.dump(reg_model, 'irrigation_regressor.pkl')
joblib.dump(label_encoder, 'label_encoder.pkl')

print("Models successfully trained and saved as .pkl files!")